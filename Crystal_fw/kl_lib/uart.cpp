...
#elif defined STM32L1XX || defined STM32F2XX || defined STM32F1XX || defined STM32F4XX
...
void BaseUart_t::Init() {
    ...
    // ==== Tx pin ====
#if defined STM32L4XX || defined STM32L1XX || defined STM32F2XX || defined STM32F4XX
    PinAF = AF7;
#if defined UART4
...
void ByteUart_t::ProcessByteIfReceived() {
    if(!RxProcessed) return;
    uint8_t b;
    while(GetByte(&b) == retvOk) {
//        PrintfI("brx: %X\r", b);
        if(Cmd.PutChar(b) == pdrNewCmd) {
            RxProcessed = false;
            EvtQMain.SendNowOrExit(EvtMsg_t(evtIdByteCmd, (ByteShell_t*)this));
            return;
        } // if new cmd
    } // while get byte
}
